...
| 症状 | 対処 |
|------|------|
| 「有効な台帳ファイルが見つかりませんでした」と表示される | アップロードした ZIP の中に `Diff List` と `Summary` の両シートを持つ `.xlsx` が存在するか確認する（ファイル名は問わない） |
| 「指番を特定できなかった台帳ファイル」と警告表示される | 台帳ファイル名が `{指番}_{モジュール}_{サイド}[_-suffix].xlsx`（例: `ME24-1001-0_ZM00_405.xlsx`）の命名規則からずれている。ファイル名を修正して再アップロードする。これらのファイルは `図形変更量詳細.xlsx` には含まれるが、`統合図面管理台帳.xlsx` の `Work Master`/`Summary`・`指番_モジュール_サイド別集計/` からは除外される |
| 「統合実行」ボタンが押せない | ZIPファイルと「統合図面管理台帳.xlsx」の両方が必須（初回は `Master` シートだけの空のExcel、1行目のヘッダーのみでよい）。「新規統合の実行」経由で統合図面管理台帳.xlsxを自動使用している場合はZIPファイルのみで押せる |
| アップロードした「統合図面管理台帳.xlsx」が反映されない | `Master` シートの名前・ヘッダー行が想定の12列と一致しているか確認する。一致しない場合は警告を表示した上で今回分のデータのみで新規作成する（`Work Master`/`Summary` シートが無い、または構成が想定と異なる場合は警告なしにそれぞれ今回分のみで作成・追記する。旧バージョン互換のための挙動） |

...
from utils.ledger_finder import RECORDED_DATE_COL as SRC_RECORDED_DATE_COL
...
# （入力図面総数・差分抽出ペア数・流用率 [%]）は含めない——図面統計は指番_モジュール_
# サイド別集計・統合図面管理台帳側の指標に一本化し、こちらの変更量詳細は図形の変更内容
# に絞っている。
...
# サイド。"Diff Type" も Diff Package から parse_diff_type() で逆算する（Master/Work
# Masterと同じ関数）。"Diff Package" 自体は参照情報として最終列に残す。
OUTPUT_HEADERS = (
...
def build_merged_workbook(entries):
    ...
    Sashiban/Module/Side を逆算できないエントリ（`parse_sashiban_module_side()`
    が None を返す場合）は末尾に回る。
    ...
    for entry in entries:
        ...
        for diff_row in entry.diff_list_rows:
            sort_key = (
                ...
                _sort_part(module), _sort_part(side), diff_row[CHILD_COL],
            )
    ...
    for _sort_key, entry, sashiban, module, side, diff_type, diff_row in flat_rows:
        ...
        row = [
            sashiban, module, side, diff_row[CHILD_COL], diff_row[PARENT_COL], diff_row[RELATION_COL],
            diff_row[TITLE_COL], diff_row[SUBTITLE_COL], diff_type,
            diff_row[DELETED_COL], diff_row[ADDED_COL], diff_row[DIFF_COL],
            diff_row[UNCHANGED_COL], diff_row[TOTAL_COL],
        ]
        ...
        row += [diff_row[NOTE_COL], diff_row[SRC_RECORDED_DATE_COL], entry.package_name]
        ws.append(row)

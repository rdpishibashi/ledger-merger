...
今回新たに得られたデータで、同じキーの既存行を上書きしつつ（新しいキーは追加）
マージする（Master・Work Masterともに同じ蓄積方式）。
...
SUMMARY_HEADERS = (
    ...
    "図形変更率 [%]", "差分ペア総数", "指番図面総数", "流用率 [%]", "日付",
)
_SUMMARY_PERCENT_LABELS = {"図形変更率 [%]", "流用率 [%]"}
_SUMMARY_COUNT_LABELS = ("削除図形総数", "追加図形総数", "変更図形総数", "図形総数", "差分ペア総数", "指番図面総数")
...
def extract_unique_work_master_rows(entries):
    ...
    for entry in entries:
        ...
        if sashiban is None:
            continue
        for row in entry.diff_list_rows:
            work_row = (sashiban,) + tuple(row[i] for i in _NON_RELATION_INDICES)
            key = (sashiban, row[_CHILD_COL], row[_PARENT_COL])
            unique.setdefault(key, work_row)
    return unique
...
def _numeric_sum(rows, col_idx):
    """rows（WORK_MASTER_HEADERS形状のタプルのリスト）の col_idx 列を合計する。
...
def compute_summary_rows(entries, run_timestamp):
    ...
    10列）を算出する。指番を逆算できないエントリは対象外（Work Masterと同じ扱い）。

    Returns:
        list[tuple]（指番昇順）
    """
    work_master_rows = extract_unique_work_master_rows(entries)
    rows_by_sashiban = {}
    for (sashiban, _child, _parent), row in work_master_rows.items():
    ...
    for sashiban in sorted(rows_by_sashiban.keys()):
        ...
        summary_rows.append((
            ...
            change_rate, pair_count, input_drawing_total, reuse_rate, run_timestamp,
        ))
    return summary_rows
...
def build_master_workbook(
    ...
    （無ければ None）をマージし（同じキーは今回のデータで上書き、無ければ追加）、
    "統合図面管理台帳.xlsx"（"Master"→"Work Master"→"Summary"の順で3シート）を
    bytes で返す。Summaryシートのみキー単位のマージは行わず、今回分の指番ごとの
    集計行を、アップロードされた前回分（previous_summary_rows、無ければ空）の
    末尾に追記する。
    """
    combined_master = dict(previous_master_rows or {})
    combined_master.update(extract_unique_child_parent_rows(entries))

    combined_work_master = dict(previous_work_master_rows or {})
    combined_work_master.update(extract_unique_work_master_rows(entries))
